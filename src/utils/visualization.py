...
def visualize_perspective_spans(question, answer, perspective_spans, output_path=None):
    ...
    for i, char in enumerate(answer):
        ...
        if current_perspectives and (
                i not in char_perspectives or set(char_perspectives[i]) != set(current_perspectives)):
            highlighted_answer += "</span>"
            current_perspectives = []

        # Check if we need to open a new span
        if i in char_perspectives and (
                not current_perspectives or set(char_perspectives[i]) != set(current_perspectives)):
